...
function buildSystemPrompt() {
  ...
  const unitList = available
    .map(
      (u) =>
        `- ${u.unit} at ${u.property} (${u.location}): ${u.type}, ${u.area.toLocaleString()} sq ft, Rs.${u.rent.toLocaleString()}/month. Floor: ${u.floor === 0 ? "Ground" : u.floor}. Amenities: ${u.amenities.join(", ")}. ${u.description} Map: ${u.mapLink}`
    )
    .join("\n");
...
async function askGemini(userMessage, conversationHistory, retries = 3) {
  ...
  return "Sorry, I'm having trouble responding right now. Please try again in a moment or contact us at mayurbothra@live.com";
}
